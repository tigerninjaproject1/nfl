...
bp1 + geom_boxplot() + ylab("Yards Per Play") + xlab("Team") + ggtitle("Median Yards per Play (by Team)") + geom_hline(yintercept = median(bygame$avgYdsPerGamePerTeam)) # Added horizontal line for the median of avgYdsPerGamePerTeam
```  
...
coefs <- coef(lm(SumYdsGain ~ SumTDs, data = ydsGainVTd))
p2 <- ggplot(ydsGainVTd, aes(SumTDs, SumYdsGain)) + geom_point() + xlab("")
p2 + xlab("Touchdowns") + ylab("Yards") + ggtitle("Touchdowns vs. Yards (Per Game)") + geom_abline(intercept = coefs[1], slope = coefs[2])
...
# Calculate intercept and slope for AB Line
coefs <- coef(lm(SDYdsGain ~ SumTDs, data = ydsGainVTd))
ggplot(ydsGainVTd, aes(SumTDs, SDYdsGain, group = "_id.GameID")) + 
  geom_point() + xlab("Touchdowns") + ylab("StDev in Yards (Per Game)") + ggtitle("Touchdowns vs. Variance in Yards Per Game") + geom_abline(intercept = coefs[1], slope = coefs[2])
```
...
# Calculate intercept and slope for AB Line
coefs <- coef(lm(SumYdsGain ~ SDYdsGain, data = ydsGainVTd))
ggplot(ydsGainVTd, aes(SDYdsGain, SumYdsGain, group = "_id.GameID")) + 
  geom_point() + ylab("Yards") + xlab("StDev(Yards)") + ggtitle("Yards vs. StDev(Yards) per Game") + geom_abline(intercept = coefs[1], slope = coefs[2])
```
